function [f,g,h,values] = cec20_func_01apr(x,prob_k)
...
if(prob_k == 1) %RC1
    ...
    values.D = 9;
    values.lb = [0,0,0,0,1000,0,100,100,100];
    values.ub = [10,200,100,200,2000000,600,600,600,900];
    values.f_optimalvalue = 1.8931162966e+2;
end
...
if(prob_k == 2) %RC2
    ...
    values.D = 11;
    values.lb = [10000,10000,10000,0,0,0,100,100,100,100,100];
    values.ub = [81.9e+4,113.1e+4,205e+4,5.074e-2,5.074e-2,5.074e-2,200,300,300,300,400];
    values.f_optimalvalue = 7.0490369540e+3;
end
...
if (prob_k == 3) %RC3
    ...
    values.D = 7;
    values.lb = [1000,0,2000,0,0,0,0];
    values.ub = [2000,100,4000,100,100,20,200];
    values.f_optimalvalue = -4.5291197395e+3;
end
...
if (prob_k == 4) %RC4
    ...
    h(:,2) = x(:,3)+x(:,1)+k3.*x(:,3).*x(:,5)-1;
    h(:,3) = x(:,2)-x(:,1)+k2.*x(:,2).*x(:,6);
    h(:,4) = x(:,4)-x(:,3)+x(:,2)-x(:,1)+k4.*x(:,4).*x(:,6);
    g(:,1) = x(:,5).^0.5+x(:,6).^0.5-4;

    values.D = 6;
    values.lb = [0,0,0,0,0.00001,0.00001];
    values.ub = [1,1,1,1,16,16];
    values.f_optimalvalue = -3.8826043623e-1;
end
...
if(prob_k == 5) %RC5
    ...
    values.D = 9;
    values.lb = zeros(1,9);
    values.ub = [100,200,100,100,100,100,200,100,200];
    values.f_optimalvalue = -4.0000560000e+2;
end
...
if(prob_k == 11) %RC11
    %% Two-reactor Problem
        ...
        z1 = 0.9*(1-exp(0.5.*w1)).*x1;
        z2 = 0.8*(1-exp(0.4.*w2)).*x2;
        
        
        f = 7.5.*y1 + 5.5.*y2 + 7.*w1 + 6.*w2 + 5.*(x1+x2);

...
[r1, r2] = gnR1R2_SASS(PopSize, size(popAll, 1), r0);
...
ui = han_boun_SASS(ui, xmax, xmin, x, PopSize,1);
%% evaluate
[fx_new,gx_new,hx_new] = EvalFunc(ui,I_fno);fx_new = fx_new'; gx_new = gx_new';hx_new = hx_new';
...
for i = 1:PopSize
    if rand < 0.2 && convx_new(i) ~= 0 && mod(gg,n) == 0 && convx_new(i) < 1e2  
          ...
          [fval, gv, hv]      = EvalFunc(new_mutant,I_fno);
          fx_new(i)           = fval;                                                               
...
%% ========================= update archive ===============================
archive = updateArchive_SASS(archive, x(I == 1, :));

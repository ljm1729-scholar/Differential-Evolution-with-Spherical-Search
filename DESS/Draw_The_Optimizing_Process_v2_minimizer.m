function Draw_The_Optimizing_Process_v2_minimizer(History,lb,ub,option,name,minimizer)
PlottingTime=char(datetime('now','TimeZone','Asia/Seoul','Format','d-MM-y (HH,mm,ss.SSS)'));
...
minimizerForVisualization=(minimizer-CenterOfDomain);
minimizerForVisualizationNormalized=minimizerForVisualization./normalizer;
minimizerForVisualizationLocaltion=minimizerForVisualizationNormalized>=0;
minimizerLocation=num2str(minimizerForVisualizationLocaltion);
minimizerLocation(minimizerLocation==' ')=[];
minimizerLocation=reshape(minimizerLocation,[],History.Dim);
minimizerLocation=bin2dec(minimizerLocation);
minimizerDeg=-UnitDeg-2*minimizerLocation*UnitDeg;
minimizerDistanceFromCOD=log(sum(minimizerForVisualizationNormalized.^2,2).^0.5)/log(History.Dim);
...
switch option
    case 'movie'
        ...
        for i_frame=1:History.iter
            ...
            hold on
            p3=polarplot(minimizerDeg,minimizerDistanceFromCOD+1.0e-8,'o','Color',[0.2,0.2,0.9],'MarkerFaceColor',[0.2,0.2,0.9],'MarkerSize',4);
            %xlabel('Objective Functional value')
            %ylabel('Total Constraint value')
            %disp(figure_text)
            drawnow
        ...
        v = VideoWriter(['[',PlottingTime,']',name,'.mp4'],'MPEG-4');
        v.FrameRate=60;
        open(v) 
        writeVideo(v,Figure_struct) 
        close(v)
        movefile(['[',PlottingTime,']',name,'.mp4'],'video')

function Draw_The_3DLeft_Optimizing_Process_v2(History,lb,ub,option,name)
...
Plot3Color = [0*ones(SASSIndex,1),0*ones(SASSIndex,1),0*ones(SASSIndex,1);...
              0*ones(NumberPopulation-SASSIndex,1),0*ones(NumberPopulation-SASSIndex,1),0*ones(NumberPopulation-SASSIndex,1)];
...
VisualizationMatrix(SASSIndex+1:SASSIndex+20,:)=[];
...
%             set(cbar,'Ticks',[0;0.5;1],'TickLabels',{"Start";"Transition";"End"})
            ...
            zlim([0 inf])
            %xlim([-inf inf])
            xlabel('$f(\vec{x})$','Rotation',45,'VerticalAlignment','bottom','HorizontalAlignment','center','Units','normalized','Interpreter','latex')

...
kappa_old=1/4;
alpha_old=1/6;
p_old=0.0228; %p_old=0.0278; %p_old=0.015; 
gamma_mild_old=1/11.7;
gamma_severe_old=1/14;
%f=0.01;
f_old=0.01/p_old;
...
%% optimization
%store_bestx=zeros(6,6);
NN=270;
ind=3;
%init_guess=[0.129357288000000,0.129357288000000,0.170366736000000,0.170366736000000,0.884814578000000,0.884814578000000,0.933249217000000,0.933249217000000,0.662157037000000,0.662157037000000,0.280364366000000,0.280364366000000,0.142545767000000,0.142545767000000,0.783126769000000,0.783126769000000,0.885922980000000,0.885922980000000,0.813213466000000,0.813213466000000,0.182306405000000,0.182306405000000,0.852069772000000,0.852069772000000,0.836151211000000,0.836151211000000,0.628464895000000,0.628464895000000,0.120148867000000,0.120148867000000,0.759953725000000,0.759953725000000];
%for i=4:5
days = 1:NN;

day_interval=NN/ind;
days_point=[1:day_interval:length(days),length(days)];


Model=@(x) ode45(@omi_ode_3rd_wave_with_C,days,X0,[],x,days_point,R_old,kappa_old,alpha_old,p_old,gamma_mild_old,gamma_severe_old,f_old,waningR);
%Y2=deval(Model(mu2),days,4);

HB=100;
Epsilon=1;
for ii=1:ps
g(ii,1)=abs(max(deval(Model(x(ii,:)),days,4)-HB))-Epsilon;
g(ii,2)=deval(Model(x(ii,:)),days(end),7)/1000000*(1-x(ii,end))*R_old-0.9;
end
h = zeros(ps,1);
...
if (prob_k == 59)
    ...
    %sin(x/2).^2 + y.*cos(y).^2.*cos(y*10).^4
    f = (sin(x(:,1)/2)).^2 + x(:,2).*(cos(x(:,2)/2)).^4;
    g = x(:,1).^2 + (x(:,2)-2).^2-4.1;
    h = zeros(ps,1);
end
